...
class NCB_BLOCK(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.blocks = nn.ModuleList([NCB(c1, c2) for _ in range(n)])
        self.shortcut = shortcut
        self.shortcut_conv = Conv(c1, c2, 1, 1) if shortcut else None # ConvBNReLU(c1, c2, kernel_size=1, stride=1, bias=False) if shortcut else None
...
class C3NCB(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(NCB_BLOCK(c_, c_) for _ in range(n)))
        
class NTBBOT(nn.Module):
    def __init__(self,c1,shortcut=True,e=0.5):
        super().__init__()
        c_ = int(c1 * e)
        self.cv1 = Conv(c1,c_,1,1)
        self.tr = NTB(c_,c_,path_dropout=0,stride=1,sr_ratio=1,
                      mlp_ratio=2,head_dim=16,mix_block_ratio=0.75,attn_drop=0,drop=0)
        self.cv2 = Conv(c_,c1,1,1)
        self.shortcut = shortcut
